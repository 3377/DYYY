...
@property (nonatomic, strong) NSArray<DYYYSettingItem *> *autoGeneratedSettings;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(settingsUpdated:)
                                               name:@"DYYYSettingsUpdated"
                                             object:nil];
    
    // 加载已保存的自动生成设置
    NSArray *savedSettings = [[NSUserDefaults standardUserDefaults] arrayForKey:@"DYYYAutoGeneratedSettings"];
    if (savedSettings) {
        [self updateAutoGeneratedSettings:savedSettings];
    }
    
    // 添加豆包按钮开关
    CGFloat currentY = self.footerLabel.frame.origin.y + self.footerLabel.frame.size.height + 20;
    UILabel *hideDoupackButtonLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, currentY, 200, 30)];
    hideDoupackButtonLabel.text = @"隐藏豆包按钮";
    [self.view addSubview:hideDoupackButtonLabel];
    
    self.hideDoupackButtonSwitch = [[UISwitch alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.view.frame) - 70, currentY, 51, 31)];
    [self.hideDoupackButtonSwitch addTarget:self action:@selector(hideDoupackButtonSwitchChanged:) forControlEvents:UIControlEventValueChanged];
    self.hideDoupackButtonSwitch.on = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYHideDoupackButton"];
    [self.view addSubview:self.hideDoupackButtonSwitch];
...
- (void)settingsUpdated:(NSNotification *)notification {
    NSArray *settings = notification.userInfo[@"settings"];
    [self updateAutoGeneratedSettings:settings];
}

- (void)updateAutoGeneratedSettings:(NSArray *)settings {
    NSMutableArray *items = [NSMutableArray array];
    for (NSDictionary *setting in settings) {
        DYYYSettingItem *item = [DYYYSettingItem itemWithTitle:setting[@"title"]
                                                         key:setting[@"key"]
                                                        type:DYYYSettingItemTypeSwitch];
        [items addObject:item];
    }
    self.autoGeneratedSettings = items;
    
    // 更新设置分组
    NSMutableArray *sections = [self.settingSections mutableCopy];
    if (sections.count >= 3) { // 假设第3组是隐藏设置组
        NSMutableArray *hideSettings = [sections[2] mutableCopy];
        [hideSettings addObjectsFromArray:self.autoGeneratedSettings];
        sections[2] = hideSettings;
...
- (void)hideDoupackButtonSwitchChanged:(UISwitch *)sender {
    [[NSUserDefaults standardUserDefaults] setBool:sender.on forKey:@"DYYYHideDoupackButton"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
